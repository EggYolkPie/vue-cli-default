...
//初始化区域图的对象
  ...
  var width = document.getElementsByClassName("router")[0].clientWidth - 20;

  var height = $(".router").height() - 50;
  this.$bgDiv.css({width: width + "px", height: height + "px"});
  this.$tool = null;//左侧工具栏对象
  this.$head = null;//顶部标签及工具栏按钮
  this.$title = "ETL流程图";//流程图的名称
  ...
  if (property.haveHead) {
    ...
    for (var x = 0; x < property.headBtns.length; ++x) {
      tmp += "<a href='javascript:void(0)' class='GooFlow_head_btn'><i data-tip='"+property.headBtns[x].split("-")[1]+"' class='ico_" + property.headBtns[x].split("-")[0] + "'></i></a>"
    }
    tmp += "</div>";
    this.$head = $(tmp);
  ...
  if (property.haveTool) {
    ...
    this.$bgDiv.append("<div class='GooFlow_tool'" + (property.haveHead ? "" : " style='margin-top:3px'") + "><p class='el-tag el-p' style='width: 100%'>组件面板</p><div style='height:" + (height - headHeight + 30 - (property.haveHead ? 7 : 10)) + "px' class='GooFlow_tool_div'></div></div>");
    ...
    if (property.toolBtns && property.toolBtns.length > 0) {
      ...
      for (var i = 0; i < property.toolBtns.length; ++i) {
        tmp += "<a href='javascript:void(0)' type='" + property.toolBtns[i] + "' id='" + this.$id + "_btn_" + property.toolBtns[i].split(" ")[0] + "' class='GooFlow_tool_btn'><i class='" + property.toolBtns[i] + "'/></a>";//加入自定义按钮
      }
  ...
  var wd = $(".GooFlow_head").width();
  this.$bgDiv.append("<div class='GooFlow_work' style='width:" + (wd) + "px;height:" + (height) + "px;" + (property.haveHead ? "" : "margin-top:3px") + "'></div>");
  ...
  if (this.$editable) {
    ...
    // })
    var flag = this;
    this.$workArea[0].addEventListener("dragover", function () {
      event.preventDefault();
    });
    this.$workArea[0].addEventListener("drop", function (event) {
      event.stopPropagation();
      if (isDown == true) {
        var ev = mousePosition(event);
        flag.$workArea.unbind("load")
        flag.$workArea.on("load", {inthis: flag}, function (e) {
          ...
          var This = e.data.inthis;
          var ind = This.$nowType.split(" ")[0];
          var index_Name = eval("This.$nodeRemark." + ind);
    ...
    //选定一条转换线后出现的浮动操作栏，有改变线的样式和删除线等按钮。<i class='b_l1' title='并发'></i>
    this.$lineOper = $("<div class='GooFlow_line_oper' style='display:none'><i class='b_x' title='删除'></i></div>");//选定线时显示的操作框
...
GooFlow.prototype = {
  ...
  initDraw: function (id, width, height) {
    ...
    $(this.$draw).delegate(tmpClk, "click", {inthis: this}, function (e) {
      ...
      This.$textArea.val(oldTxt).css({
        ...
        if (e.button == 2) return false;
        console.log(watchDouble)
        if(watchDouble==true) {
          This.setName(This.$textArea.data("id"), "||", "line");
        }
  ...
  setNodeRemarks: function (remark) {
    ...
    this.$tool.children("a").each(function () {
      ...
      this.innerHTML = "<i class='ico_" + $(this).attr("type") + "'></i>" + remark[$(this).attr("id").split("btn_")[1]];
    });
  ...
  switchToolBtn: function (type) {
    // console.log(this.$tool.children("#" + this.$id + "_btn_" + type.split(" ")[0]).attr("class", "GooFlow_tool_btndown")[0])
    ...
    var move_div = this.$tool.children("#" + this.$id + "_btn_" + type.split(" ")[0]).attr("class", "GooFlow_tool_btndown")[0];//要操作的div对象
    ...
    move_div.onmouseup = function (event) {
      isDown = false;
    }
    if (this.$nowType == "group") {
      this.blurItem();
  ...
  addNode: function (id, json) {
    ...
    if (json.type.indexOf(" round") < 0) {
      ...
      json.width = 24;
      json.height = 24;
      this.$nodeDom[id] = $("<div   class='showTag GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='0'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span' style='cursor: pointer'>" + json.name + "</div></div>");
    }
  ...
  initWorkForNode: function () {
    ...
    this.$workArea.delegate(".ico", "mousedown", {inthis: this}, function (e) {
      ...
      document.onmousemove = function (e) {
        ...
        This.$ghost.css({left: X - 220 + hack + "px", top: Y + hack + "px"});
        isMove = true;
  ...
  delNode: function (id) {
    ...
    // if(!_del.$nodeData[id])	return;
    // if(_del.onItemDel!=null&&!_del.onItemDel(id,"node"))	return;
    //先删除可能的连线
    for (var k in _del.$lineData) {
      if (_del.$lineData[k].from == id || _del.$lineData[k].to == id) {
        _del.$draw.removeChild(_del.$lineDom[k]);
        delete _del.$lineData[k];
        delete _del.$lineDom[k];
        _del.delLine(k);
      }
    }
    //再删除结点本身
    if (_del.$undoStack) {
      var paras = [id, _del.$nodeData[id]];
      _del.pushOper("addNode", paras);
    }
    delete _del.$nodeData[id];
    _del.$nodeDom[id].remove();
    delete _del.$nodeDom[id];
    --_del.$nodeCount;
    if (_del.$focus == id) _del.$focus = "";

    if (_del.$editable) {
      //在回退新增操作时,如果节点ID以this.$id+"_node_"开头,则表示为本次编辑时新加入的节点,这些节点的删除不用加入到$deletedItem中
      if (id.indexOf(_del.$id + "_node_") < 0)
        _del.$deletedItem[id] = "node";
    }
...
///////////以下为有关画线的方法
  ...
  watchDouble: function (id) {//監控多線程
    watchDouble=true;
    if($("#"+id).find("text").html()=="||"){
      $("#"+id).find("text").html("")
    }else{
      $("#"+id).find("text").html("||")
    }
  ...
  delArea: function (id) {
    ...
    if(this.$areaDom[id]==undefined)return;
  ...
  addArea: function (id, json) {
    ...
    if (this.$group != null) {
      this.$group.append(this.$areaDom[id][0]);
    }
